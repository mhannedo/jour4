...
def six1(i)
    return i.count {|x| x[/_/]}
    #compte le nombre d'emails avec l'underscope et non les underscore
end

def six2(i)
    count =[]
    i.each {|x| count x.scan(/_/).length}
    return count.length
     #a revoir
end
...
def huit (i)
    ...
    newliste= i.sort_by(&:length)
    return newliste[1..50]

end

def neuf(i)

return i.index("@epenser")+1

end

def perform (i)
puts "Quelle question ?"
